...
void draw() {
  ...
  if (printing) {
    text("Printing!", width/2 - offset, height/2);
    t.playAudio("https://www.drivehq.com/file/DFPublishFile.aspx/FileID5033468331/Key64rsqbthpgqt/dogebark.wav");
  } else {
    text("Not Printing!", width/2 - offset, height/2);
  }
}
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkTypetag("fffff")){
    float ffval1 = theOscMessage.get(0).floatValue(); 
    println("ffVal1: " + ffval1);
    if(ffval1 < 4){
      t.makeVibrate();
